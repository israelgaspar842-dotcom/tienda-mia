...
import { Gift, Cog, ShieldCheck, Clock3, Layers } from "lucide-react";
  ...
  try {
    ...
    if (!error && data) {
      proyectos = (data as HeroProject[]).map((row) => ({
        ...
        titulo: String((row as Record<string, unknown>)["titulo"] ?? ""),
        descripcion: String((row as Record<string, unknown>)["descripcion"] ?? ""),
        imagen_url: (row as Record<string, unknown>)["imagen_url"] as string | null,
      }));
    }
  } catch {
    proyectos = [];
    ...
    <section className="relative overflow-hidden bg-transparent dark:bg-zinc-950">
      ...
      <div className="mx-auto max-w-7xl px-6 py-10 md:py-16">
        <div className="grid lg:grid-cols-[1.45fr_1fr] gap-8 lg:gap-10 items-center">
          {/* Carrusel — reemplaza imagen estática de la guitarra */}
          <div className="order-1 lg:order-2 relative">
            <div className="relative">
              <div className="absolute inset-0 rounded-[28px] bg-orange-500/20 blur-2xl translate-y-3 translate-x-1" />
              {/* ESTRUCTURA DEL CARRUSEL: contenedor principal aspecto ventana */}
              <div className="w-full relative rounded-2xl overflow-hidden">
                <div className="border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 rounded-2xl overflow-hidden">
                  <HeroCarousel proyectos={proyectos} />
                  <div className="absolute inset-0 rounded-2xl border border-white/[0.04] pointer-events-none" />
                </div>
              </div>
            </div>
            <p className="mt-3 text-center text-[11px] text-slate-500 dark:text-neutral-500 font-medium">
              Desliza para explorar • {proyectos.length > 0 ? `${proyectos.length} destacados` : "Capas 0.2mm • Relleno 20%"}
            </p>
          </div>
          ...
          <div className="order-2 lg:order-1 space-y-6">
            <Badge
              variant="secondary"
              className="gap-2 py-1.5 px-3 rounded-full bg-white dark:bg-neutral-800 border-slate-200 dark:border-neutral-700 text-slate-700 dark:text-neutral-300"
            >
            ...
            <div className="grid sm:grid-cols-2 gap-4 pt-2">
              ...
              >
                <span className="absolute top-3 right-3 text-[10px] font-black tracking-widest uppercase bg-orange-500/15 text-orange-600 dark:text-orange-400 border border-orange-500/20 px-2 py-1 rounded-full">
                  Ideal regalos
                </span>
                ...
                <div className="space-y-1.5">
                  <h3 className="text-[15px] font-black tracking-tight text-slate-900 dark:text-white group-hover:text-orange-500 transition-colors">
                    Imprimir una idea o regalo
                  </h3>
                  <p className="text-xs text-slate-600 dark:text-neutral-400 leading-relaxed">
                    Figuras y decoración. Pega tu link y elige color. Sin jerga.
                  </p>
                </div>
                <span className="flex items-center gap-2 text-xs font-bold text-slate-900 dark:text-white">
                  Cotizar mi regalo
                  <span className="h-6 w-6 rounded-full bg-slate-200 dark:bg-neutral-700 text-slate-700 dark:text-neutral-200 flex items-center justify-center text-[11px] group-hover:bg-orange-500 group-hover:text-white transition-colors">
                    →
                  </span>
                </span>
              </Link>
              ...
              <Link
                ...
                className="group relative flex flex-col justify-between p-6 rounded-2xl bg-white border border-slate-200 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 dark:bg-zinc-900/50 dark:border-zinc-800 dark:shadow-none dark:hover:border-neutral-700"
              >
                <span className="absolute top-3 right-3 text-[10px] font-black tracking-widest uppercase bg-slate-200 dark:bg-neutral-700 text-slate-700 dark:text-neutral-300 border border-slate-300 dark:border-neutral-600 px-2 py-1 rounded-full">
                  NDA incluido
                </span>
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-slate-600 dark:text-neutral-400 leading-relaxed">
                    Piezas funcionales, tolerancias y confidencialidad.
                  </p>
                </div>
                <span className="flex items-center gap-2 text-xs font-bold text-slate-900 dark:text-white">
                  Cotizar prototipo
                  <span className="h-6 w-6 rounded-full bg-slate-200 dark:bg-neutral-700 border border-slate-300 dark:border-neutral-600 text-slate-700 dark:text-neutral-200 flex items-center justify-center text-[11px] group-hover:bg-orange-500 group-hover:text-white group-hover:border-orange-500 transition-colors">
                    →
                  </span>
                </span>
              </Link>
